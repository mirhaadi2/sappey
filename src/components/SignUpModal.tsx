...
import {
  X, User, Envelope, Phone, ArrowLeft,
  Lock, ShieldCheck, WarningCircle, CaretRight
            ...
            <div className="flex justify-between items-start mb-10">
              <div>
                ...
                <div className="flex gap-1.5 mt-2">
                  ...
                  <div className={`h-1 w-6 rounded-full transition-all duration-300 ${step === 'password' ? 'bg-brand-brown' : 'bg-brand-brown/20'}`} />
                </div>
              </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === "info" && (
                <motion.form key="info" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                  onSubmit={handleSubmit(handleSendOtp)} className="space-y-5"
                >
                  <div className="mb-4">
                    ...
                    <div key={f.id} className="relative group">
                      ...
                      {errors[f.id as keyof RegisterData] && (
                        <p className="text-[11px] text-red-500 mt-1.5 ml-1 font-bold flex items-center gap-1">
                          <WarningCircle weight="fill" /> {errors[f.id as keyof RegisterData]?.message}
                        </p>
                      )}
                    </div>
                  ))}

                  <button type="submit" disabled={loading} className="w-full py-3 bg-brand-brown text-white rounded-2xl font-bold shadow-xl shadow-brand-brown/20 hover:bg-brand-brown/90 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              {step === "otp" && (
                ...
                >
                  <button onClick={() => setStep("info")} className="flex items-center gap-2 text-brand-brown font-bold text-sm mb-6 hover:opacity-70 transition-opacity">
                  ...
                  <button onClick={handleVerifyOtp} disabled={loading || otpValue.join("").length < 6} className="w-full py-4.5 bg-brand-brown text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-brand-brown/90 shadow-xl shadow-brand-brown/10 disabled:opacity-50 transition-all">
                    {loading ? <Spinner /> : "Verify Email"}
                  </button>

                  <div className="mt-8">
                    {timer > 0 ? (
                      <p className="text-sm text-gray-400 font-medium">Resend code in <span className="text-brand-brown font-bold">{timer}s</span></p>
                    ) : (
                      <button onClick={() => { handleSendOtp(userData); }} className="text-sm text-brand-brown font-bold hover:underline transition-all">Resend Code</button>
                    )}
                  </div>
                </motion.div>
              )}
              ...
              {/* STEP 3: PASSWORD */}
              {step === "password" && (
                <motion.div key="password" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8" >
                  <div>
                    <h3 className="text-2xl font-bold text-gray-800 mb-2">Secure Account</h3>
                    <p className="text-gray-500">Create a password to complete your profile.</p>
                  </div>

                  <div className="space-y-4">
                    <div className="relative group">
                      <Lock size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-brand-brown" />
                      <input
                        type="password"
                    ...
                    <div className="px-1">
                      <div className="flex gap-1.5 h-1.5 mb-2">
                        {[1, 2, 3, 4].map((l) => (
                          <div key={l} className={`h-full flex-1 rounded-full transition-all duration-500 ${strength >= l ? (strength <= 1 ? "bg-red-400" : strength === 2 ? "bg-orange-400" : strength === 3 ? "bg-yellow-400" : "bg-green-500") : "bg-gray-100"}`} />
                        ))}
                      </div>
                      <div className="flex justify-between text-[11px] font-bold uppercase tracking-wider">
                        <span className="text-gray-400">Security Strength</span>
                        <span className={strength <= 1 ? "text-red-500" : strength === 2 ? "text-orange-500" : strength === 3 ? "text-yellow-600" : "text-green-600"}>
                          {["Too Weak", "Weak", "Fair", "Strong", "Rock Solid"][strength]}
                        </span>
                      </div>
                    </div>
                  </div>
