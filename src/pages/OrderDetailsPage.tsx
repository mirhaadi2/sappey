    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] py-12 px-4 sm:px-8">
            <div className="max-w-6xl mx-auto">
                ...
                >
                    {/* Background Accent - Very subtle leaf or pattern feel */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-brand-cream/20 rounded-bl-full -z-0" />
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-3">
                                <span className="font-label text-[10px] font-black uppercase tracking-[0.3em] text-brand-brown/40">
                                    Harvest Tracking Info
                                </span>
                                <div className={`w-2 h-2 rounded-full animate-pulse ${order.status === 'DELIVERED' ? 'bg-emerald-500' : 'bg-orange-500'}`} />
                            </div>

                            <h1 className="font-headline text-3xl font-black text-brand-brown tracking-tighter mb-2 ">
                                Order Number: {order.orderNumber}
                            </h1>

                            <p className="font-sans text-slate-500 font-bold text-sm flex items-center gap-2">
                                <Clock weight="bold" className="text-brand-brown/30" />
                                Confirmed on {new Date(order.createdAt).toLocaleDateString("en-US", { dateStyle: 'full' })}
                            </p>
                        </div>

                        {/* Status Badge Wrapper - Made it look like a "Seal of Quality" */}
                        <div className="flex items-center p-1.5 rounded-full bg-brand-cream/30 border border-brand-brown/5 shadow-inner">
                            <OrderStatusBadge
